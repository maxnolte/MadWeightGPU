...
__copyright__= "Copyright (c) 2008-2011 MadGraph/MadEvent team"
__license__= "GNU"
##
## BEGIN CODE
##
...
class single_machine(cluster):
    ...
    class create_file(cluster.create_file):
        ...
        def one(self,directory,nbcard,nbevent=-1,dir_type='P',prog='madevent.py',argument=''):
            try:
                self.mother.launch.one(directory,nbcard,nbevent,dir_type,prog)
            except:
                pass
#            print 'pass create file'
    ...
    class launch(cluster.launch):
        ...
        def one(self,directory,nbcard,nbevent=-1,dir_type='P',prog=''):
            ...
            if self.MWparam['mw_run']['write_log']:
                ...
                print 'Launching the program now!: '+prog
		os.system(prog)
...
class condor(cluster):
    ...
    class create_file(cluster.create_file):
        ...
        def condor_submission_file(self,pos,prog,posdir,nbsubmit=1,argument=''):
            ...
            condor_req=self.MWparam.condor_req #supress the ''
        ...
        def Card(self,directory,nb_card,dir_type):
            ...
            if dir_type=='M':
                self.condor_submission(directory,self.MWparam.nb_event_MW[directory],nb_card,dir_type)
            else:
        ...
        def one(self,directory,nbcard,nbevent=-1,dir_type='P',prog='madevent.py',argument=''):
            ...
            pos=os.path.abspath('./SubProcesses/'+directory) #to check
            dir_name=self.MWparam.name
            # Pierre: Xsections are not evaluated anymore
            #if dir_type=='P':
    ...
    class create_file(cluster.create_file):
        ...
        def all_MadWeight(self,main=0):
            ...
            for directory in self.MWparam.MW_listdir:
                for i in range(0,self.MWparam.nb_event_MW[directory]):
...
class SGE(cluster):
    ...
    class create_file(cluster.create_file):
        ...
        standard_text=open('./Source/MadWeight/tools/sge_schedular','r').read()
        #3 ##################################################################	
        #def Card(self,directory,nb_card,dir_type):
        #    """ creates the submission for all the event for a given card in a specific directory """
        #    pass	
...
#	    print self,directory,nbcard,nbevent,dir_type
...
class Bsub(cluster):
    ...
    class create_file(SGE.create_file):
        ...
        standard_text=open('./Source/MadWeight/tools/bsub_schedular','r').read()
...
class QSUB(cluster):
    ...
    tag=5 # value to call this class from MadWeight_card.dat
    ...
    class create_file(cluster.create_file):
       ...
       #3 ##################################################################				
	    ...
	    pos=os.path.realpath(os.getcwd())
            ...
            text="cd "+posdir+'/\n'
            if self.MWparam['mw_run']['write_log']:
                self.output = posdir+'/err\n'
                self.error = posdir+'/out\n'
            else:
    ...
    class launch(cluster.launch):
        ...
        def __init__(self,mother):
            ...
            self.submitted = 0 # limiting submission
            self.waiting_submission = [] # file not yet submitted

        #3 ##############################################################################################
        def launch_one_submission_file(self,file):
            """ launch one specific submission file """

            if self.submitted < self.MWparam['mw_run']['max_submission']:
                self.submitted +=1
            else:
                self.waiting_submission.append(file)
                if self.pbar:
        ...
        def all(self):                                                                            
            ...
            while 1:                                                                              
                ...
                if self.MWparam.norm_with_cross:                                                  
                    self.all_MadEvent(main=0)                                                     
                print self
                if self.to_launch+self.idle+self.running==0: 
                    break     
                nb_waiting = len(self.mother.launch.waiting_submission) * self.MWparam.n_join + 50
                if self.idle < nb_waiting and self.mother.launch.waiting_submission:
                    print 'Restart a submission of job'
                    data = list(self.mother.launch.waiting_submission)
                    self.mother.launch.waiting_submission = []
                    self.mother.launch.submitted = 0
                    for file in data:
                        self.mother.launch.launch_one_submission_file(file)
...
#########################################################################
#   QSUB cluster  -submission by bigger packet-					  		#
#########################################################################
class QSUB2(QSUB,cluster):

    tag=51 # value to call this class from MadWeight_card.dat
    ...
    class create_file(cluster.create_file):
        ...
        def packet(self,directory,nbcard,nbevent,dir_type='P',prog="madevent.py"):
            ...
            text = "#!/bin/sh\n"
            posdir=pos+"/SubProcesses/"+directory[0]+'/'
            text+="cd "+posdir+'/\n'
            text+="date\n"
        ...
        def one(self,directory,nbcard,nbevent=-1,dir_type='P',prog='madevent.py',opt='',argument=''):
            ...
            self.packet([directory],[nbcard],[nbevent],[dir_type],prog)
            
    class launch(QSUB.launch):
        submit_word = 'qsub'
